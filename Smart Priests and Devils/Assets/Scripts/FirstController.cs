...
public class FirstController : MonoBehaviour, ISSActionCallback, ISceneController, IUserAction
{
    ...
    public void Hint()
    {
        ...
        State current = new State(priests[(int) BoatPosition.Left].Count, devils[(int) BoatPosition.Left].Count,
            priests[(int) BoatPosition.Right].Count, devils[(int) BoatPosition.Right].Count,
            boatPosistionState == BoatPosition.Right ? 1 : -1);
        List<Action> actions = new List<Action>();
        foreach (var state in policy.validStates)
        {
            if (state.Equals(current))
            {
                foreach (var nextState in state.nextStates)
                {
                    actions.Add(nextState.Key);
                }
                break;
            }
        }

        policyAction = actions[random.Next(actions.Count)];
        autoState = AutoState.ToBoat;

        switch (policyAction)
        {
            case Action.P:
                autoState = AutoState.MoveBoat;
                PriestToBoat();
                break;
            case Action.PD:
            case Action.PP:
    ...
    public void SSActionEvent(SSAction action)
    {
        ...
        {
            if (autoState == AutoState.MoveBoat)
            {
                MoveBoat();
                autoState = AutoState.ToBank;
            }
            else if (autoState == AutoState.ToBoat)
            {
                switch (policyAction)
                {
                    case Action.PP:
            ...
            else if (autoState == AutoState.ToBank)
            {
                switch (policyAction)
                {
                    case Action.P:
                    ...
                    case Action.PD:
                        PriestToBank();
                        policyAction = Action.D;
                        break;
                    case Action.PP:
                        PriestToBank();
                        policyAction = Action.P;
                        break;
                    case Action.DD:
                        DevilToBank();
                        policyAction = Action.D;
